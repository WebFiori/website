...
class WebFioriAPITheme extends APITheme {
    ...
    public function createMethodDetailsBlock($func){
        ...
        $node->addChild($descNode);
        if($count != 0){
            $paramsNode = WebFioriAPIGUI::createRowNode(FALSE,FALSE);
        ...
        if($return !== NULL){
            $retNode = WebFioriAPIGUI::createRowNode(FALSE,FALSE);
            $textNode = new Paragraph();
