...
class UnorderedListView extends P {
    public function __construct(){
        parent::__construct();
        $this->setTheme('webfiori\theme\NewFioriAPI');
        $this->getTheme()->setBaseURL('https://webfiori.com/docs');
        $this->setDescription('A class that represents Unordered List HTML element (ul)');
        $this->setWebsiteName('WebFiori API Docs');
        $this->setTitle('class UnorderedList');
        $this->insert($this->getTheme()->createClassDescriptionNode('class', 'UnorderedList', '\webfiori\ui', 'A class that represents Unordered List HTML element (ul) '));
        $classAttrsArr = [
        ];
        $classMethodsArr = [
            new FunctionDef([
